...
def article(request):
	...
	query = 'SELECT article_id, title, content, credibility_score, publisher, published_date, article_link FROM TWEET_DATA order by credibility_score LIMIT 30;'
...
def articles_detail(request, article_number):
	...
	cursor = mydb.cursor()
	mydb.set_character_set('utf8mb4')
	query = 'SELECT article_id, title, content, publisher, published_date, credibility_score, article_link FROM TWEET_DATA WHERE article_id = ' + article_number + 'order by credibility_score LIMIT 30;'
	print(query)
